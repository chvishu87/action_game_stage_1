...
IOManager::IOManager( ) :
  ...
  MAX_STRING_SIZE( gdata->getXmlInt("maxStringSize") ),
    ...
    screen(SDL_SetVideoMode(viewWidth, viewHeight, 0, SDL_HWSURFACE)),		// The screen is released automatically by SQL_Quit
...
SDL_Surface* IOManager::loadAndSet(const string& filename, bool setcolorkey) const {
  ...
  if ( setcolorkey ) {
    Uint32 colorkey = SDL_MapRGB(tmp->format, 255, 255, 255);	// changed second color argument from 0 to 255
